...
C23456789012345678901234567890123456789012345678901234567890123456789012
	...
	REAL*16  omega(3),Ueigval(3),eigvec(3,3)
      ...
      REAL*16 S0y, my, Vy, e0y, Hy, ny
      REAL*16 sEqNew, eReq, Ry, Siy, Srefy, erefy, eReqRatio, Sy, PLASTIC
...
!     PROPS 45      : D01    % Prony First Coefficient  
!     PROPS 46 - 66 : Dj1    % Prony Coefficients
!     PROPS 67      : V    % V activation volume
!
!     Cooperative yield model properties
!     PROPS 68      : S0y    % Internal stress at 0K
!     PROPS 69      : my    % Parameter m
!     PROPS 70      : Vy    % Activation volume for Cooperative model
!     PROPS 71      : e0y    % Pre-exponential strain rate
!     PROPS 72      : Hy    % Activation energy for Cooperative model
!     PROPS 73      : ny    % Parameter n
...
!     STATEV 165 : Sy            % yield stress
!     STATEV 166 : PLASTIC       % If entered plasticity (1) or no (0) according to von Mises
!
!***************************   Material parameters  **************************** 
...
!   Obtain compliance Prony coefficients 
      ...
      do j=1,NProny
        tauj(j)=PROPS(23+j)
		S11j(j)=PROPS(45+J)
		S12j(j)=-Poisson*(PROPS(45+j))
		S21j(j)=-Poisson*(PROPS(45+j))
        S22j(j)=PROPS(45+j)
		S66j(j)=PROPS(45+j)*((1+Poisson))
	    S13j(j)=S12j(j)
        S23j(j)=S12j(j)
      end do
...
!   Obtain instantaneous compliance 
	  S110=PROPS(45)
	  S120=-Poisson*(PROPS(45))
	  S210=-Poisson*(PROPS(45))
	  S220=PROPS(45)
	  S660=PROPS(45)*((1+Poisson))
	  S130=-Poisson*PROPS(45)
	  S230=-Poisson*PROPS(45)
      
!   Read Cooperative yield model material parameters	
      S0y=PROPS(68)
      my=PROPS(69)
      Vy=PROPS(70)
      e0y=PROPS(71)
...
!     Calculate equivalent strain rate
      eReq = (STATEV(163)-oldEqStr)/DTIME
      STATEV(164) = eReq

!   Calculate yield stress according to Cooperative model
      Ry = 8.3145
      Siy = S0y - my * Temperature
      Srefy = 2 * Ry * Temperature / Vy
      erefy = e0y * exp(-Hy / Ry / Temperature)
      eReqRatio = (eReq / erefy) ** (1 / ny)
      Sy = Siy + Srefy * LOG(eReqRatio + SQRT(eReqRatio ** 2 + 1))

!   If strain rate lower than threshold (unloading/relaxation/recovery) use previous yield stress
      IF (eReq.lt.1.D-20) THEN
        Sy = STATEV(165)
      END IF
      
!   If yield stress lower than 0 (extremely low strain rate), then set yield stress as 0
      IF (Sy.lt.0) THEN
        Sy = 0
      END IF
      STATEV(165) = Sy
      
!   Check if has entered plasticity according to von Mises yield criterion
      PLASTIC = 0
      IF (sEqNew .gt. Sy) THEN
        PLASTIC = 1
      END IF
      STATEV(166) = PLASTIC
